...
class Journal():  # シート固有の値。
	def __init__(self):
		...
		self.daycolumn = 1  # 取引日列インデックス。
		self.splittedcolumn = 3  # 固定列インデックス。
	def setSheet(self, sheet):  # 逐次変化する値。
		self.sheet = sheet
		cellranges = sheet[self.splittedrow:, self.daycolumn].queryContentCells(CellFlags.DATETIME)  # 取引日列の文字列が入っているセルに限定して抽出。
